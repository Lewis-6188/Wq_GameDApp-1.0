...
import view.wanq.com.ptr_lib.R;
...
public class PtrFrameLayout extends ViewGroup {
    ...
    public PtrFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (arr != null) {
            ...
            mPtrIndicator.setResistanceHeader(
                    arr.getFloat(R.styleable.PtrFrameLayout_ptr_resistance, mPtrIndicator.getResistanceHeader()));
            mPtrIndicator.setResistanceFooter(
                    arr.getFloat(R.styleable.PtrFrameLayout_ptr_resistance, mPtrIndicator.getResistanceFooter()));
            mPtrIndicator.setResistanceHeader(arr.getFloat(R.styleable.PtrFrameLayout_ptr_resistance_header,
                    mPtrIndicator.getResistanceHeader()));
            mPtrIndicator.setResistanceFooter(arr.getFloat(R.styleable.PtrFrameLayout_ptr_resistance_footer,
                    mPtrIndicator.getResistanceFooter()));

            mDurationToBackHeader = arr.getInt(R.styleable.PtrFrameLayout_ptr_duration_to_back_refresh,
                    mDurationToCloseHeader);
            mDurationToBackFooter = arr.getInt(R.styleable.PtrFrameLayout_ptr_duration_to_back_refresh,
                    mDurationToCloseHeader);
            mDurationToBackHeader = arr.getInt(R.styleable.PtrFrameLayout_ptr_duration_to_back_header,
                    mDurationToCloseHeader);
            mDurationToBackFooter = arr.getInt(R.styleable.PtrFrameLayout_ptr_duration_to_back_footer,
                    mDurationToCloseHeader);

            mDurationToCloseHeader = arr.getInt(R.styleable.PtrFrameLayout_ptr_duration_to_close_either,
                    mDurationToCloseHeader);
            mDurationToCloseFooter = arr.getInt(R.styleable.PtrFrameLayout_ptr_duration_to_close_either,
                    mDurationToCloseFooter);
            mDurationToCloseHeader = arr.getInt(R.styleable.PtrFrameLayout_ptr_duration_to_close_header,
                    mDurationToCloseHeader);
            mDurationToCloseFooter = arr.getInt(R.styleable.PtrFrameLayout_ptr_duration_to_close_footer,
                    mDurationToCloseFooter);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (DEBUG && DEBUG_LAYOUT) {
            PtrCLog.d(LOG_TAG, "onMeasure frame: width: %s, height: %s, padding: %s %s %s %s", getMeasuredWidth(),
                    getMeasuredHeight(), getPaddingLeft(), getPaddingRight(), getPaddingTop(), getPaddingBottom());
        ...
        if (mContent != null) {
            ...
            if (getLayoutParams().height == LayoutParams.WRAP_CONTENT) {
                super.setMeasuredDimension(getMeasuredWidth(),
    ...
    private void layoutChildren() {
        // because the header and footer can not show at the same time, so when header has a offset, the footer's offset
        // should be 0, vice versa..
        int offsetHeaderY;
        int offsetFooterY;
        ...
        if (DEBUG && DEBUG_LAYOUT) {
            PtrCLog.d(LOG_TAG, "onLayout offset: %s %s %s %s", offsetHeaderY, offsetFooterY, isPinContent(),
        ...
        if (mHeaderView != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                PtrCLog.d(LOG_TAG, "onLayout header: %s %s %s %s %s", left, top, right, bottom,
        ...
        if (mContent != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                PtrCLog.d(LOG_TAG, "onLayout content: %s %s %s %s %s", left, top, right, bottom,
        ...
        if (mFooterView != null) {
            ...
            if (DEBUG && DEBUG_LAYOUT) {
                PtrCLog.d(LOG_TAG, "onLayout footer: %s %s %s %s %s", left, top, right, bottom,
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                if (!mForceBackWhenComplete) {
                    ...
                    boolean isFooter = !mPtrIndicator.isHeader() && mPtrIndicator.hasLeftStartPosition(); // if the
                                                                                                          // footer is
                                                                                                          // showing
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                boolean canMoveUp = mPtrIndicator.isHeader() && mPtrIndicator.hasLeftStartPosition(); // if the header
                                                                                                      // is showing

                boolean canMoveDown = mFooterView != null && !mPtrIndicator.isHeader()
                        && mPtrIndicator.hasLeftStartPosition(); // if the footer is showing

                boolean canHeaderMoveDown = mPtrHandler != null
                        && mPtrHandler.checkCanDoRefresh(this, mContent, mHeaderView) && (mMode.ordinal() & 1) > 0;
                boolean canFooterMoveUp = mPtrHandler != null && mFooterView != null // The footer view could be null,
                                                                                     // so need double check
                        && mPtrHandler instanceof PtrHandler2
                        && ((PtrHandler2) mPtrHandler).checkCanDoLoadMore(this, mContent, mFooterView)
                        && (mMode.ordinal() & 2) > 0;
                ...
                if (DEBUG) {
                    PtrCLog.v(LOG_TAG,
                            ...
                            canHeaderMoveDown, canFooterMoveUp);
                }
                ...
                if (mPtrIndicator.isOverOffsetToKeepHeaderWhileLoading() && !stayForLoading) {
                    if (mPtrIndicator.isHeader()) {
                        mScrollChecker.tryToScrollTo(mPtrIndicator.getOffsetToKeepHeaderWhileLoading(),
                                mDurationToBackHeader);
                    } else {
                        mScrollChecker.tryToScrollTo(mPtrIndicator.getOffsetToKeepHeaderWhileLoading(),
                                mDurationToBackFooter);
                    }
                } else {
                    // do nothing
                }
    ...
    private void tryScrollBackToTop() {
        if (!mPtrIndicator.isUnderTouch() && mPtrIndicator.hasLeftStartPosition()) {
            mScrollChecker.tryToScrollTo(PtrIndicator.POS_START,
                    mPtrIndicator.isHeader() ? mDurationToCloseHeader : mDurationToCloseFooter);
            return;
        }
    ...
    public void autoRefresh(boolean atOnce, boolean isHeader) {
        ...
        mScrollChecker.tryToScrollTo(mPtrIndicator.getOffsetToRefresh(),
                isHeader ? mDurationToCloseHeader : mDurationToCloseFooter);
        if (atOnce) {
            mStatus = PTR_STATUS_LOADING;
            performRefresh();
        }
